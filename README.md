...
> [!NOTE]
> 이 저장소는 `개념 → 구성 → 검증 → 장애 분석` 순서로 읽는 학습 교재입니다. 화면에 보이는 링크는 모두 같은 목차 기준에서 생성됩니다.
...
> [!TIP]
> 한 장을 읽을 때는 먼저 장 지도의 목표를 확인하고, 코드 블록을 실습한 뒤 `show`, 로그, 테스트 결과로 검증하십시오. 다음 장은 현재 장의 검증 결과를 전제로 이어집니다.
...
- [2026-09-09 OSSEC·Snort 2.9.20 공식 보강](notes/2026-09-09_OSSEC-Snort-공식보강.md)
